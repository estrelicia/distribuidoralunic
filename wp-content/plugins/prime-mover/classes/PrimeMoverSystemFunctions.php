...
{    
     ...
     * @param number $blog_id
     * @return void|boolean|boolean|\Codexonics\PrimeMoverFramework\classes\[]
     */
    public function primeMoverDoDelete($path_supplied = '', $authorization_required = true, $start_time = 0, $blog_id = 0)
    {
        ...
        $base_export_path = $this->getSystemInitialization()->getMultisiteExportFolderPath();
        if (empty($path_supplied)) {
            $path_supplied = $base_export_path;
        }
        
        $whitelisted_targets = [
            $base_export_path,
            $this->getSystemInitialization()->getDefaultImportFolder(),
            $this->getSystemInitialization()->getLockFilesFolder(),
            $this->getSystemInitialization()->getCliMustUsePluginPath(),
            $this->getSystemInitialization()->getTmpDownloadsFolder(),
        ...
        if ($blog_id > 0) {
            $whitelisted_targets[] = $this->getProfileDirectory([], true, $blog_id);
            $whitelisted_targets[] = $this->getProfileDirectory([], false, $blog_id);
        }
        
        $real_target = realpath($path_supplied);
        if ($real_target === false) {
            return false;
        }
        
        $real_target_norm = wp_normalize_path($real_target);
        $is_safe = false;
        foreach ($whitelisted_targets as $allowed_path) {
            if (empty($allowed_path)) {
                continue;
            }
            
            $real_base = realpath($allowed_path);
            if ($real_base !== false) {
                $real_base_norm = wp_normalize_path($real_base);
                if ($real_target_norm === $real_base_norm) {
                    $is_safe = true;
                    break;
                }
                
                if (is_dir($real_base)) {
                    $real_target_dir = trailingslashit($real_target_norm);
                    $real_base_dir   = trailingslashit($real_base_norm);
                    
                    if (strpos($real_target_dir, $real_base_dir) === 0) {
                        $is_safe = true;
                        break;
                    }
                }
    ...
    public function primeMoverValidateFootprintData($footprint = [])
    {
        ...
        $error_string = '';
        $default_keys = ['plugins', 'stylesheet', 'template', 'using_child_theme', 'footprint_blog_id', 'site_url', 'wp_root', 'db_prefix'];
        
        if (isset($footprint['all_plugins'])) {
            $default_keys[] = 'all_plugins';
        }
        
        if (isset($footprint['all_themes'])) {
            $default_keys[] = 'all_themes';
        }
        
        $correct_keys = apply_filters('prime_mover_footprint_keys', $default_keys, $footprint);
    ...
    {
        return $this->getSystemInitialization()->isWpFileSystemUsable($wp_filesystem);
    }
     ...
     * @param boolean $force
     */
    public function enableMaintenanceDuringImport($force = false)
    {
        if ( ! is_multisite() ) {
            return;
        }
        ...
        if (false === $force && apply_filters('prime_mover_bypass_maintenance_mode', false)) {
            return;
        }
     ...
     */
    public function getSiteTableQuery($db = '', $wpdb = null, $target_prefix = '', $regex = '')
    {       
    ...
    {
        $this->getSystemInitialization()->initializeFs($admin_check);
    }
    ...
    /**
     * Maybe disabled export import buttons - no permissions mode.
     * @param number $blog_id
     * @return string
     */
    public function maybeDisableExportImportNoPermissions($blog_id = 0)
    {
        if (!$blog_id) {
            return '';
        }
    ...
    /**
     * Maybe show permission issues on tooltip
     * @param number $blog_id
     * @param string $mode
     * @return string
     */
    public function maybeShowPermissionIssuesOnToolTip($blog_id = 0, $mode = 'export')
    {
        if (!$blog_id || !$mode) {
            return '';
        }
    ...
    /**
     * Return export directory path with permission issues to fix
     * Empty string if nothing to fix
     * @param number $blog_id
     * @return string
     */
    public function getExportDirectoryPermissionPathToFix($blog_id = 0)
    {
        if (!$blog_id && !is_multisite()) {
            $blog_id = 1;
        }
        ...
        $parent_dir = $this->getSystemInitialization()->getMultisiteExportFolder();
        $child_dir = '';
        if ($blog_id) {
            $child_dir = $this->getExportPathOfThisSubsite($blog_id);
        }        
        
        if ($parent_dir) {
            $parent_dir = trailingslashit(wp_normalize_path($parent_dir));
        }
        
        if ($child_dir) {
            $child_dir = trailingslashit(wp_normalize_path($child_dir));
        }
       
        $dirname = '';
        if ($parent_dir) {
            $dirname = dirname($parent_dir);
        }
        
        if ($dirname) {
            $dirname = trailingslashit(wp_normalize_path($dirname));
        }
        
        if (!$child_dir && $parent_dir && $dirname) {
        ...
        if ($this->checkIfDirectoryHasPermissionIssues($parent_dir)) {    
            $this->getSystemInitialization()->setRootBackupDirUnwritable($blog_id, true); 
            return $parent_dir;
        }
        
        if ($this->checkIfDirectoryHasPermissionIssues($child_dir)) {
            $this->getSystemInitialization()->setRootBackupDirUnwritable($blog_id, true);
            return $child_dir;
        }
     ...
     * @return boolean
     */
    protected function checkIfDirectoryHasPermissionIssues($path = '')
    {
        return ($path && is_dir($path) && !wp_is_writable($path));
    }
    
    /**
     * Called only on a few Prime Mover pages where backward compatibility with legacy code is required.
     */
    public function maybeDeregisterHeartBeat()
    {
        add_filter('doing_it_wrong_trigger_error', function($ret, $function_name, $message, $version) {
            if ('WP_Scripts::add' === $function_name && '6.9.1' === $version) {
                return false;
            }
            return $ret;   
        }, 10, 4);
    ...
    /**
     * Get subsite query regex that includes stray core mu tables
     * @param wpdb $wpdb
     * @param string $target_prefix
     * @return string
     */
    protected function getSubsiteQueryRegEx($wpdb = null, $target_prefix = '')
    {
        $global_core_mu_tables = array_values($wpdb->tables('ms_global', false, 0));        
        $regex_array = [];
        foreach ($global_core_mu_tables as $table) {
            $regex_array[] = $target_prefix . $table . '$';
        }
        
        return implode("|", $regex_array);         
    }
     ...
     * @return boolean|string
     */
    public function getProfileDirectory($ret = [], $avatar_mode = true, $blog_id = 0)
    {
        if (!is_array($ret)) {
            return false;
        }
        ...
        if (!$base_dir && $blog_id) {
            ...
            $this->restoreCurrentBlog($blog_id);
        }
    ...
    public function getTablesToExport($blogid_to_export = 0)
    {
        ...
        $wpdb = $this->getSystemInitialization()->getWpdB();
        $target_prefix = $wpdb->prefix;
        $origin_target_prefix = $target_prefix;
